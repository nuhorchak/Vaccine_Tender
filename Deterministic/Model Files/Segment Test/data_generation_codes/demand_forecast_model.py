...
- Medium_Demand:        Final demand by antigen (10 forecast periods)
...
def parse_medium_demand(sheets):
    """Parse the Medium_Demand sheet - 10 forecast periods × 13 antigens."""
    df = sheets['Medium_Demand']
    # row 0 = period headers (1–10), rows 1–12 = antigens
    antigens = []
    data = {}
    for _, row in df.iterrows():
        label = row.iloc[0]
...
FORECAST_PERIODS = 10  # periods labelled 1–10 (maps to years 2021–2030)
...
def project_series(historical_vals, n_periods=10, window=5):
    """
...
# Mapping from the short-code vaccine key (used in Medium_Demand_calc)
# to its contributing vaccines in total_capacity_calcs
DEMAND_VACCINE_MAP = {
    'M':          ['Measles'],
    'MR':         ['MR'],
    'MMR':        ['MMR', 'MMRV'],
...
def aggregate_demand_by_antigen(cap_df, projections):
    ...
    Returns dict: antigen -> list of 10 projected values.
    """
    antigen_demand = {a: np.zeros(FORECAST_PERIODS) for a in ALL_ANTIGENS}

    # Get all vaccine names present in capacity data
    ...
    for vcode, contributing_vaccines in DEMAND_VACCINE_MAP.items():
        ...
        for vaccine_name in contributing_vaccines:
            # Sum across all income groups for this vaccine
...
def _build_demand_calc_sheet(wb, antigen_demand, actual_md):
    ...
    for _, row in src.iterrows():
        ...
        vals = [float(row.iloc[i]) if pd.notna(row.iloc[i]) else 0.0 for i in range(1, 11)]
    ...
    for vac in vac_order:
        ...
        vals = actual_vac.get(vac, antigen_demand.get(vac, [0]*10))
        for p_idx, val in enumerate(vals[:10]):
    ...
    for antigen in ALL_ANTIGENS:
        ...
        vals = antigen_demand.get(antigen, [0]*10)
        for p_idx, val in enumerate(vals[:10]):
            c = apply_table_style(ws, row_num, p_idx + 2, val, is_alt=is_alt, formula=True)
        row_num += 1
...
def _build_medium_demand_sheet(wb, scenario_data, actual_md):
    ...
    ws.merge_cells('A1:L1')
    t = ws['A1']
    t.value = 'Medium Demand Forecast – Antigen-Level (10 Forecast Periods)'
    t.font = Font(bold=True, color='FFFFFFFF', size=12, name='Arial')
    t.fill = make_fill(HEADER_FILL)
    t.alignment = Alignment(horizontal='center', vertical='center')
    ws.row_dimensions[1].height = 22
    ...
    sub = ws['A2']
    sub.value = ('Demand in doses. Columns 1–10 represent forecast periods. '
    ...
    for antigen in display_antigens:
        ...
        if actual_md is not None and antigen in actual_md.index:
            vals = actual_md.loc[antigen].values[:10]
        else:
            vals = medium_data.get(antigen, [0]*10)
    ...
    notes = [
        ...
        '• Source data: Demand_Scenarios_Base.xlsx | Periods 1-10 = 2021-2030',
    ]
    for note in notes:
