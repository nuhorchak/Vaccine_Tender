...
L_shaped_output = Dict()
    ...
    elseif model_type == "L-shaped-phase2"
        ...
        close(f)
    elseif model_type == "MVP"
        source = string(results_dir, "/MVP_DE_results_T_", tmax, "_delta_",max_tender_length,"_scen_",length(random_scenarios),"_trial_",trial,"_inv_",initial_inventory_rate,"_cap._",scaled_capacity,"_cap.inc._",allowable_capacity_increase_number,".json")
        f = open(source, "w")
        JSON.print(f, L_shaped_output)
        close(f)
    end
        ...
        for v in V
            ...
            for ω in Ω_test
                ...
                for t in T
                    ...
                    elseif  model_type == "DE model" || model_type == "MVP"
