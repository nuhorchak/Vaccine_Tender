...
# pi => unvaccinated chuldren penalty cost. Default value is 10. Smaller numbers can be tried for the sensitivity analysis.

# tender_stochastic_sensitivity_multi_obj(10,5,2,2,1,1,1,2,10)
function tender_stochastic_sensitivity_multi_obj(max_horizon_length,max_tender_length,number_of_demand_scenarios,total_capacity_scenarios,number_of_trials,initial_inventory_rate,scaled_capacity,allowable_capacity_increase_number,pi)
    ...
    for trial in 1:number_of_trials
        ...
        source = string(current_directory, "/results/log_DE_L_T_", max_horizon_length, "_delta_", max_tender_length, "_scen_", number_of_demand_scenarios * total_capacity_scenarios, "_trial_", trial, "_inv_", initial_inventory_rate, "_cap._", scaled_capacity, "_cap.inc._", allowable_capacity_increase_number, "_sensitivity_max_profit_min_unvax.json")
        ...
        if !isfile(source)
            ...
            current_directory = @__DIR__
            source_2 = string(current_directory, "/results/scenarios_", tmax, "_delta_", max_tender_length, "_scen_", number_of_demand_scenarios * total_capacity_scenarios, "_trial_", trial, "_inv_", initial_inventory_rate, "_cap._", scaled_capacity, "_cap.inc._", allowable_capacity_increase_number, "_sensitivity_max_profit_min_unvax.json")
        ...
        function save_L_shaped_results(F_bar, Y_bar, W_bar, L_bar, Q_bar, X_de, I_de, Vc_de, S_de)
            ...
            current_directory = @__DIR__
            source = string(current_directory, "/results/DE_results_T_", tmax, "_delta_", max_tender_length, "_scen_", number_of_demand_scenarios * total_capacity_scenarios, "_trial_", trial, "_inv_", initial_inventory_rate, "_cap._", scaled_capacity, "_cap.inc._", allowable_capacity_increase_number, "_sensitivity_max_profit_min_unvax.json")
            f = open(source, "w")
        ...
        function deterministic_equivalent(p_ω, Ω)
            ...
            if capacity_extension_decision
                @objective(model, Max, sum(p_ω[ω] * r[v, p, t] * X[v, p, t, ω] / delta[t] for v in V, p in P_v[v], t in T, ω in Ω) 
                                        - sum(Y[p, t] * f_profit[v, p, t] / delta[t] for p in P, v in V_p[p], t in T)
        ...
        current_directory = @__DIR__
        source = string(current_directory, "/results/log_DE_T_", tmax, "_delta_", max_tender_length, "_scen_", number_of_demand_scenarios * total_capacity_scenarios, "_trial_", trial, "_inv_", initial_inventory_rate, "_cap._", scaled_capacity, "_cap.inc._", allowable_capacity_increase_number, "_sensitivity_max_profit_min_unvax.json")
