...
gurobi_solver_DE = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "FeasibilityTol" => 1e-4, "OutputFlag" => 1, "Presolve" => 1, "NumericFocus" => 1, "MIPGap" => 1e-3, "Threads" => 8, "TimeLimit" => 20) 
...
function tender_stochastic_sensitivity(max_horizon_length,max_tender_length,number_of_demand_scenarios,total_capacity_scenarios,number_of_trials,initial_inventory_rate,scaled_capacity,allowable_capacity_increase_number, extension_decision = true)
    ...
    for trial in 1:number_of_trials
        ...
        capacity_extension_decision = extension_decision
        ...
        current_directory = @__DIR__
        filename = "data/Starting_point2.xlsx"
        relative_path = joinpath(current_directory, filename)
        ...
        function save_L_shaped_results(F_bar, Y_bar, W_bar, L_bar, Q_bar, X_de, I_de, Vc_de, S_de)
            ...
            for (t, tau) in F_time_set
                for a in A
                    if (a, t, tau) ∉ starting_points_vect_F
                        tender_cost += g[t] * F_bar[a, (t, tau)] / delta[t] 
                    end
                end
            end
            println("tender_cost: $(unit*tender_cost)")
            ...
            for v in V
                for p in P_v[v]
                    for t in T
                        for ω in Ω_test
                            purchase_cost += unit*p_ω_test[ω] * r[v, p, t] * X_de[v, p, t, ω] / delta[t]
                        end
                    end
                end
            end
            println("expected_purchase_cost: $purchase_cost")
...
tender_stochastic_sensitivity(10,5,2,2,1,1,1,2, false)