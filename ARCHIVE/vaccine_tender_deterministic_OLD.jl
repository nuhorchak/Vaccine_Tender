...
for v in V
    for p in P_v[v]
        for t in T
            for tau in T
                if (t,tau) in F_time_set
                    @constraint(model, Q[v,p,(t,tau)] >= W[p,(t,tau)]*sum(X[v,p,l] for l in t:tau))
                end
            end
        end
    end
...
for p in P
    for t in T
        @constraint(model, sum(r[v,p,t]*X[v,p,t] for v in V_p[p]) >= sum((1+l[v,p])*f[v,p,t]*Y[p,t] for v in V_p[p]))
    end
end
