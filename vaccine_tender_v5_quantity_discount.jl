...
price_status = "P2" # "P2" => "P_constant_discount" or "P3" => "P_increasing_discount"
...
#println(XLSX.sheetnames(demand_file))
d_real_raw = demand_file["Medium_Demand"]
#d_real_raw = demand_file["Pandemic_Scenario"]
...
#println(XLSX.sheetnames(capacity_file))
...
for p in P
    for v in V_p[p]
        for t in T
            f[v,p,t] = s_real[p,t]/length(V_p[p])*r_producer_avg[p]/4
        end
    end
end
...
# γ: discount factor for each quantity segment
γ = Dict()
if price_status == "P2"
    for m in M
        γ[m] = 0.02*(m-1)
    end
elseif price_status == "P3"
    inc_pricing = [0.0, 0.0063, 0.0125, 0.025, 0.05, 0.10]
    for m in M
        γ[m] = inc_pricing[m]
    end
end
...
@variable(model, X[v in V, p in P_v[v], t in T, m in M] >= 0)
@variable(model, Z[v in V, p in P_v[v], t in T, m in M], Bin)
...
@objective(model, Min, sum(g[t]*F[a,(t,tau)] for (t,tau) in F_time_set, a in A)
                            + sum((1-γ[m])*r[v,p,t]*X[v,p,t,m] for v in V, p in P_v[v], t in T, m in M)
                                + sum(pi*r_avg[v,t]*S[a,t] for v in V, a in A, t in T)
                                    + sum(h[v]*r_avg[v,t]*I[v,t] for v in V, t in T)
                                        #+ 100000*(sum(Q[v,p,(t,tau)] for v in V, p in P_v[v], (t,tau) in F_time_set) - sum(X[v,p,t] for v in V, p in P_v[v], t in T))
                                                                                    )
        ...
        for t in T
            for tau in T
                if (t,tau) in F_time_set
                    @constraint(model, (tau-t+1)*F[a,(t,tau)] <= sum(Y[v,p,l] for l in t:tau, p in P_v[v]))
                end
            end
        end
    end
...
for p in P
    for t in T
        for tau in T
            if (t,tau) in F_time_set
                @constraint(model, sum(Q[v,p,(t,tau)] for v in V_p[p]) <= sum(s_real[p,l] for l in t:tau))
            end
        end
    end
end
    ...
    for a in A_v[v]
        for p in P_v[v]
            for t in T
                for tau in T
                    if (t,tau) in F_time_set
                        @constraint(model, Q[v,p,(t,tau)] >= F[a,(t,tau)]*sum(X[v,p,l,m] for l in t:tau, m in M))
                    end
                end
            end
        end
...
for v in V
    for p in P_v[v]
        for t in T
            @constraint(model, sum(X[v,p,t,m] for m in M) <= s_real[p,t]*Y[v,p,t])
        end
    end
end
...
for v in V
    for t in T
        if t >= tmin
            @constraint(model, I[v,t-1] + sum(X[v,p,t,m] for p in P_v[v], m in M) == Vc[v,t] + I[v,t])
        end
    end
end
...
for p in P
    for t in T
        @constraint(model, sum((1-γ[m])*r[v,p,t]*X[v,p,t,m] for v in V_p[p], m in M) >= sum((1+l[v,p])*f[v,p,t]*Y[v,p,t] for v in V_p[p]))
    end
end
...
for v in V
    for p in P_v[v]
        for t in T
            for m in M
                @constraint(model, X[v,p,t,m] >= q_L[m]*Z[v,p,t,m])
            end
        end
    end
end
...
for v in V
    for p in P_v[v]
        for t in T
            for m in M
                if m in M
                    @constraint(model, X[v,p,t,m] <= q_U[m]*Z[v,p,t,m])
                end
            end
        end
    end
...
source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/F_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
F_result = Dict()
for a in A
    temp_1 = Dict()
    for t in T
        ...
        for tau in T
            if (t,tau) in F_time_set
                if JuMP.value.(model[:F][a,(t,tau)]) >= 1.0
                    temp_2[tau] = JuMP.value.(model[:F][a,(t,tau)])
                    #F_result[a][t][tau] = JuMP.value.(model[:F][a,t,tau])
                end
            end
        end
        temp_1[t] = temp_2
...
JSON.print(f, F_result)
close(f)

source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/Tender_length_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
count_length_1 = 0
count_length_3 = 0
count_length_5 = 0
...
Tender_len = Dict()
Tender_len["1"] = count_length_1
Tender_len["3"] = count_length_3
Tender_len["5"] = count_length_5
JSON.print(f, Tender_len)
close(f)

source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/Y_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
Y_results = Dict()
for v in V
    temp_1 = Dict()
    for p in P_v[v]
        temp_2 = Dict()
        for t in T
            if JuMP.value.(model[:Y][v,p,t]) >= 1.0
                temp_2[t] = JuMP.value.(model[:Y][v,p,t])
            end
        end
        temp_1[p] = temp_2
    end
...
JSON.print(f, Y_results)
close(f)

source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/Q_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
Q_results = Dict()
for v in V
    temp_1 = Dict()
    for p in P_v[v]
        ...
        for t in T
            ...
            for tau in T
                if (t,tau) in F_time_set
                    if JuMP.value.(model[:Q][v,p,(t,tau)]) >= 1.0
                        temp_3[tau] = JuMP.value.(model[:Q][v,p,(t,tau)])
                    end
                end
            end
            temp_2[t] = temp_3
...
JSON.print(f, Q_results)
close(f)

source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/X_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
X_results = Dict()
for v in V
    temp_1 = Dict()
    for p in P_v[v]
        ...
        for t in T
            val = 0.0
            for m in M
                if JuMP.value.(model[:Z][v,p,t,m]) >= 1.0
                    val = JuMP.value.(model[:X][v,p,t,m])
                end
            end
            temp_2[t] = val
        end
...
JSON.print(f, X_results)
close(f)

source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/I_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
I_results = Dict()
for v in V
    temp_1 = Dict()
    for t in T
        if JuMP.value.(model[:I][v,t]) >= 1.0
            temp_1[t] = JuMP.value.(model[:I][v,t])
        end
    end
    I_results[v] = temp_1
end
JSON.print(f, I_results)
close(f)


source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/Vc_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
Vc_results = Dict()
for v in V
    temp_1 = Dict()
    for t in T
        if JuMP.value.(model[:Vc][v,t]) >= 1.0
            temp_1[t] = JuMP.value.(model[:Vc][v,t])
        end
    end
    Vc_results[v] = temp_1
end
JSON.print(f, Vc_results)
close(f)

source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/S_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
S_results = Dict()
for a in A
    temp_1 = Dict()
    for t in T
        if JuMP.value.(model[:S][a,t]) >= 1.0
            temp_1[t] = JuMP.value.(model[:S][a,t])
        end
    end
    S_results[a] = temp_1
end
JSON.print(f, S_results)
close(f)

source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/Vc_antigen_results_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
Vc_antigen_results = Dict()
for a in A
    temp_1 = Dict()
    for t in T
        temp_value = 0.0
        for v in V_a[a]
            temp_value += JuMP.value.(model[:Vc][v,t])
        end
        if temp_value >= 1
            temp_1[t] = temp_value
        end
    end         
    Vc_antigen_results[a] = temp_1
end
JSON.print(f, Vc_antigen_results)
close(f)

source = string("C:/Users/fthcn/Desktop/Vaccine_Tender_Scheduling_Problem/Julia_Files/Github/Results/model_objective_and_run_time_",demand_status,"_",supply_status,"_",price_status,"_discount.json")
f = open(source, "w")
model_result = Dict()
model_result["objective_value"] = JuMP.objective_value(model)
model_result["solve_time"] = JuMP.solve_time(model)
JSON.print(f, model_result)
close(f)
