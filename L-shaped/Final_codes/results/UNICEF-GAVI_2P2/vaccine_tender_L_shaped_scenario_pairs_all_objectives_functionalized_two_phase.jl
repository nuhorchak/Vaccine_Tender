...
function tender_stochastic_sensitivity(
    ...
    for trial in 1:number_of_trials
        ...
        while iter1 <= iter_max
            ...
            #             @constraint(Masterproblem, trust_region_constr_all, trust_expr_all >= 1)
            #         end
            #     end
            # end
            ...
            if feasible_solution == false
                println("INFEASIBLE SOLUTION: Previous solution was optimal, all others pruned")
            else
                println("Feasible soltion found and saved, moving to phase 2")
            end
        ...
        while iter2 <= iter_max 
            ...
            if iter2 >= 2 && (mip_gap > min_gap)
                ...
                try
                    ...
                    if mip_gap >= min_gap
                        ...
                        trust_expr_W = sum(
                            W_bar[p, (t, tau)] == 1 ? (1 - Masterproblem[:W][p, (t, tau)]) :
                                                    Masterproblem[:W][p, (t, tau)]
                            for p in P, (t, tau) in F_time_set
                        )
            ...
            if (UB - LB) / abs(UB) < relaxation_tol2 || gap_counter > 10 || iter2 >= iter_max 
