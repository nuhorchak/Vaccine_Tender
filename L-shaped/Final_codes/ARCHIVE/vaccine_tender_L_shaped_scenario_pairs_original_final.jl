...
function tender_stochastic_sensitivity(max_horizon_length,max_tender_length,number_of_demand_scenarios,total_capacity_scenarios,number_of_trials,initial_inventory_rate,scaled_capacity,allowable_capacity_increase_number, cap_ext_bool = false)
    ...
    for trial in 1:number_of_trials
        ...
        # current_directory = '/scrfs/storage/uhorchak/home/model/Model_objective_analysis'
        ...
        # source = joinpath(current_directory, "results", "log_DE_L_T_", max_horizon_length, "_delta_", max_tender_length, "_scen_", number_of_demand_scenarios * total_capacity_scenarios, "_trial_", trial, "_inv_", initial_inventory_rate, "_cap._", scaled_capacity, "_cap.inc._", allowable_capacity_increase_number".json")
        println("Source file located:")
        println(source)
        # # Define the directory and file path
        # results_directory = joinpath(current_directory, "results")
        ...
        if !isfile(source)
            ...
            # open(source, "w") do file
            #     # You can write initial content or leave it empty
            #     write(file, "{}")  # Writing an empty JSON object as an example
            # end

            overlap_decision = true
            capacity_extension_decision = cap_ext_bool
            # max_tender_length = 5
            ...
            while iter <= iter_max
                ...
                if (UB - LB) / UB < relaxation_tol
                    ...
                    # output lines for SLURM job
                    # Print the values of the variables
                    println("F_de: ", value.(F_de))
                    println("Y_de: ", value.(Y_de))
                    println("W_de: ", value.(W_de))
                    println("L_de: ", value.(L_de))
                    println("Q_de: ", value.(Q_de))
                    println("X_de: ", value.(X_de))
                    println("I_de: ", value.(I_de))
                    println("Vc_de: ", value.(Vc_de))
                    println("S_de: ", value.(S_de))
                    println("model_type: ", model_type)  # Assuming 'model_type' is not a JuMP variable but a normal variable
...
# function tender_stochastic_sensitivity(max_horizon_length, max_tender_length, number_of_demand_scenarios, total_capacity_scenarios, number_of_trials,
    # initial_inventory_rate, scaled_capacity, allowable_capacity_increase_number, cap_ext_bool)