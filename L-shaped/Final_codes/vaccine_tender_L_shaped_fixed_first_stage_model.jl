...
include(joinpath(functions_directory, "master_problem_fix_first_stage.jl"))
...
gurobi_solver = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "FeasibilityTol" => 1e-2, "OutputFlag" => 1, "Presolve" => 0, "NumericFocus" => 1, "Heuristics" => 0.5, "PumpPasses" => 10, "GomoryPasses"=>2)#, "Threads" => 8) 
...
function tender_stochastic_sensitivity(
    ...
    for trial in 1:number_of_trials
        ...
        Masterproblem = master_problem_fixed_first(Masterproblem, A, F_time_set, V, P_v, P, T, L_lower_number, L_upper_number, 
        ...
        println("################################ Solving fixed first stage MILP... ###############################")
               
        while iter2 <= iter_max
            println("Fixed iteration: $iter2")
            ...
            if (UB - LB) / abs(UB) < relaxation_tol2
                println("Fixed First Stage converged in $time_elapsed seconds after $iter2 iterations")
