...
if capacity_extension_decision
    ...
    + sum(p_ω[ω]*r[v,p,t]*X[v,p,t,ω]/(1+delta[t]) for v in V, p in P_v[v], t in T, ω in Ω)
        + sum(p_ω[ω]*pi*S[a,t,ω]/(1+delta[t]) for a in A, t in T, ω in Ω)
            + sum(p_ω[ω]*h[v]*r_avg[v,t]*I[v,t,ω]/(1+delta[t]) for v in V, t in T, ω in Ω)
