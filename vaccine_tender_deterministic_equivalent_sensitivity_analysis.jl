...
gurobi_solver = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "FeasibilityTol"=>1e-6, "TIME_LIMIT" => 60.0) #"TIME_LIMIT" => 20.0
...
# Constraint - Fix the schedule or not
for a in A
    for t in T
        for tau in T
            if (t,tau) in F_time_set
                if (t,tau) == (1,5) || (t,tau) == (6,10)
                    @constraint(model, F[a,(t,tau)] == 1)
                else
                    @constraint(model, F[a,(t,tau)] == 0)
                end
            end
        end
    end
...
println("Total tender number: $total_tender_number")
println("Average tender length: $avg_tender_length")
# println(Tender_len)
...
for p in P
    ...
    for t in T
        for v in V_p[p]
            for ω in Ω
                temp_amount = temp_amount + p_ω[ω]*JuMP.value.(model[:X][ω,v,p,t])
            end
        end
    end
...
println("Number of producers participated in tenders: $producer_participated_in_tender_number out of total number of producers: $(length(P))")
# println(Producer_production_check)

vaccine_preference_MMR_incl_ant = Dict()
vaccine_preference_Hexa_incl_ant = Dict()
vaccine_preference_Others = Dict()
V = ["M","MR","MMR","TT","HepB","Hib","IPV","OPV","DT","Td","DTwP","DTwP-Hib","Penta","Hexa","HPV","Rotavirus","PCV"]
for v in V
    temp_amount = 0.0
    for t in T
    ...
    if v == "M" || v == "MR" || v == "MMR"
        vaccine_preference_MMR_incl_ant[v] = temp_amount
    elseif v == "HPV" || v == "Rotavirus" || v == "PCV" 
        vaccine_preference_Others[v] = temp_amount
    else
        vaccine_preference_Hexa_incl_ant[v] = temp_amount
    end
end
println(vaccine_preference_MMR_incl_ant)
println(vaccine_preference_Hexa_incl_ant)
println(vaccine_preference_Others)

producer_capacity_usage = Dict()
for p in P
    temp_amount = 0.0
    total_capacity = 0.0
    for t in T
        ...
        total_capacity += s_real[p,t]
    end
    producer_capacity_usage[p] = temp_amount/total_capacity
end
println(producer_capacity_usage)

producer_portfolio_usage = Dict()
for p in P
    temp = Dict()
    for v in V_p[p]
        temp_amount = 0.0
        ...
        temp[v] = temp_amount/total_capacity
    end
    producer_portfolio_usage[p] = temp
end
# println(producer_portfolio_usage)

total_inventory = 0.0
for v in V
    for t in T
        for ω in Ω
            total_inventory += p_ω[ω]*JuMP.value.(model[:I][ω,v,t])
        end
    end
end
println("total inventory: $total_inventory")

total_unvaccinated_children = 0.0
for a in A
    for t in T
        for ω in Ω
            total_unvaccinated_children += p_ω[ω]*JuMP.value.(model[:S][ω,a,t])
        end
    end
end
println("total unvaccinated children: $total_unvaccinated_children")