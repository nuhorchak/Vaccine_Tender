...
gurobi_solver = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "FeasibilityTol" => 1e-6, "OutputFlag" => 0, "Presolve" => 0, "NumericFocus" => 1)#, "Heuristics" => 0.5, "PumpPasses" => 10, "GomoryPasses"=>2)#, "Threads" => 8) 
gurobi_solver_DE = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "FeasibilityTol" => 1e-6, "OutputFlag" => 1, "Presolve" => 1, "NumericFocus" => 1)#, "MIPGap" => 1e-1)#, "Threads" => 8) 
gurobi_solver_no_presolve = JuMP.optimizer_with_attributes(Gurobi.Optimizer, "FeasibilityTol" => 1e-6, "OutputFlag" => 0, "Presolve" => 0, "NumericFocus" => 1)#, "MIPGap" => 1e-1)#, "Threads" => 8) 
    ...
    for trial in 1:number_of_trials
        ...
        while iter1 <= iter_max
            ...
            if termination_status(Masterproblem) == MOI.OPTIMAL
                ...
                for ω in Ω_test_partial_2
                    println("SOLVING MMW DUAL SUBPROBLEM")
                    ...
                    if status in (MOI.INFEASIBLE, MOI.INFEASIBLE_OR_UNBOUNDED, MOI.DUAL_INFEASIBLE)
                        ...
                        error("Terminating after conflict.")
                    end
                    ...
                    Z_MMW_omega[ω] = JuMP.objective_value(dualsubproblem)
                end #end dual solving

                println("Generating Cuts")
    
                # #generate benders cuts
                Masterproblem = generate_cuts_from_dual(Masterproblem, dual_subproblem_MMW, Ω_test_partial_2, V, P_v, T, F_time_set, 
                X_tilde_upper, P, s_real_tilde, 1, A, d_real_tilde, l, f_profit, V_p, I_hat, S_hat, 
            ...
            # generate cut for UB using epsilon
                # epsilon = max(5e-5 * abs(UB), 1)  # use 1e-3, not 10e-4
                ...
                # update UB
                # global Z_S_expected = sum(p_ω_test_partial_2[ω] * Z_S_omega[ω] for ω in Ω_test_partial_2)
                global Z_MMW_expected = sum(p_ω_test_partial_2[ω] * Z_MMW_omega[ω] for ω in Ω_test_partial_2)
...
tender_stochastic_sensitivity(10,5,2,2,1,1,1,2, true, true, true, false, false)