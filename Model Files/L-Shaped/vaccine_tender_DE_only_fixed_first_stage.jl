...
elseif SOCIAL_BENEFIT_MODEL
    ...
    for trial in 1:number_of_trials
        ...
        Deterministic_Equivalent = deterministic_equivalent_fix_first_stage(1, 1, g, beta, Γ, gurobi_solver_DE,
        A, A_p, F_time_set, V, P_v, T, T_initial, P, P_a, starting_points_vect_F, starting_points_vect_I, 
        starting_points_vect_S, capacity_extension_decision, UNICEF_MODEL, L_lower_number, L_upper_number,
        κ, s_real, L_hat_upper, L_check_upper, d_real_tilde, X_tilde_upper, s_real_tilde, 1, 
        ...
        # test save with new format
        F_detailed = Dict()
        for a in A
            temp = Dict()
            for t in T, tau in T
                if (t, tau) in F_time_set && (a, t, tau) ∉ starting_points_vect_F
                    temp["$(t)_$(tau)"] = g[t] * value(Deterministic_Equivalent[:F][a, (t, tau)]) / delta[t]
                end
            end
            F_detailed[a] = temp
        end
                ...
                for t in T, tau in T
                    if (t, tau) in F_time_set
                        ...
                        for m in keys(m_segments)
                            q_val = value(Deterministic_Equivalent[:Q][v, p, (t, tau), m])
                            if q_val > 0
                                temp_tau["$(m)"] = r_avg[v, t] * (1 - zeta_vm[v, m]) * q_val / delta[t]
                            end
                        end
                        if !isempty(temp_tau)
                            temp_p["$(t)_$(tau)"] = temp_tau
                        end
                    end
                end
                if !isempty(temp_p)
        ...
        source = joinpath(results_dir, "manual_objective_components.json")
        open(source, "w") do f
            JSON.print(f, obj_components; indent=4)
        end
...
tender_stochastic_sensitivity(10,5,1,1,1,1,1,1, true, true, true, false, false, 0.02)