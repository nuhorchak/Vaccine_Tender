...
"""
function load_model_starting_points_OLD(file_path::String, initial_inventory_rate::Int, unit::Int, A, V)
    # Read the Excel file
    ...
    # column_names = string.(vec(starting_points_F_raw[1, :]))  # Convert to 1D vector of Strings
    # data = starting_points_F_raw[2:end, :]
    # F_df = DataFrame(data, column_names)
    # for row in eachrow(F_df)
    #     if row.Antigen in A
    #         push!(starting_points_vect_F, (row.Antigen, row.Starting, row.Ending))
    #     end
    # end
    ...
    starting_points_vect_I = Any[]
    # column_names = string.(vec(starting_points_I_raw[1, :]))  # Convert to 1D vector of Strings
    # data = starting_points_I_raw[2:end, :]
    # I_df = DataFrame(data, column_names)
    # for row in eachrow(I_df)
    #     if row.Vaccine in V
    #         push!(starting_points_vect_I, (row.Vaccine, row.Amount))
    #     end
    # end
    ...
    # column_names = string.(vec(starting_points_S_raw[1, :]))  # Convert to 1D vector of Strings
    # data = starting_points_S_raw[2:end, :]
    # S_df = DataFrame(data, column_names)
    # for row in eachrow(S_df)
    #     if row.Antigen in A
    #         push!(starting_points_vect_S, (row.Antigen, row.Unvaccinated_Children))
    #     end
    # end
